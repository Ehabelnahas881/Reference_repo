...
def ensure_connection():
    """Checks if the global connection is alive; reconnects if not."""
    global SHARED_CONN
    try:
        # This is the fastest way to check if a Postgres connection is still alive
        with SHARED_CONN.cursor() as cur:
            cur.execute("SELECT 1")
    except (psycopg2.OperationalError, psycopg2.InterfaceError, Exception):
        logger.info("Database connection lost. Reconnecting...")
        try:
            try: SHARED_CONN.close()
            except: pass
            SHARED_CONN = psycopg2.connect(**db_config)
            SHARED_CONN.autocommit = True
            logger.info("Database connection restored.")
        except Exception as e:
            logger.error(f"Critical: Could not restore DB connection: {e}")
            
SHARED_CONN = None

def start_global_connection():
    global SHARED_CONN
    while SHARED_CONN is None:
        try:
            SHARED_CONN = psycopg2.connect(**db_config)
            SHARED_CONN.autocommit = True
            logger.info("Global Connection established.")
        except Exception as e:
            logger.error(f"DB Startup failed: {e}. Retrying in 10s...")
            time.sleep(10)

start_global_connection()
...
class AsyncDBWriter:
    """Manages non-blocking database storage using the shared connection."""
    ...
    def _writer_loop(self):
        while True:
            ...
            try:
                # 1. Check connection before attempting work
                ensure_connection() 
                ...
                self.db_log.info(f"SUCCESS | {ticker} | {full_timestamp}")

            except Exception as e:
                # 2. Protection: If writing fails, put data BACK in the queue so it's not lost
                self.db_log.error(f"Write failed for {ticker}: {e}. Re-queueing.")
                self.queue.put((ticker, row))
                time.sleep(5) # Brief pause to allow internet to stabilize
            finally:
                self.queue.task_done()

# Re-initialize the global writer using the shared connection
db_writer = AsyncDBWriter(SHARED_CONN)

def validate_asset_market_day(ticker):
    """Checks trading day using the SHARED_CONN without reconnecting."""
    try:
        # Use the global SHARED_CONN instead of creating a new one
        with SHARED_CONN.cursor() as cur:
            query = """
                SELECT 
        ...
        return False
    # No finally block closing the connection!
...
def asset_worker_thread(ticker):
    ...
    worker_log.info(f"[{ticker}] Worker initialized.")
...
def get_symbols_from_pks(pk_list):
    """Translates PKs into symbols using the SHARED_CONN."""
    if not pk_list:
        return []
    
    try:
        # No psycopg2.connect() here anymore!
        with SHARED_CONN.cursor() as cur:
            query = f"""
