...
# SCALED TO 10 ASSETS
ASSETS = [
    {"ticker": "TSLA"}, {"ticker": "AAPL"}, {"ticker": "NVDA"},
    {"ticker": "MSFT"}, {"ticker": "AMZN"}, {"ticker": "GOOGL"},
    {"ticker": "META"}, {"ticker": "AMD"}, {"ticker": "NFLX"}, {"ticker": "INTC"}
]
...
class StealthOHLCWorker:
    ...
    async def process_asset(self, session, asset):
        ticker = asset['ticker']
        logger.info(f"Worker {ticker} Active.")
        ...
        while True:
            try:
                ...
                # FIXED: 6 TICKERS PER MINUTE
                for _ in range(6):
                    p = await self.fetch_price(session, ticker)
                    if p: prices.append(p)
                    # Jitter: ~9 seconds between hits to look human
                    await asyncio.sleep(random.uniform(8.5, 9.5))
                
                if prices:
                    o, h, l, c = prices[0], max(prices), min(prices), prices[-1]
                    logger.info(f"[{session_name}] {ticker} O:{o:.2f} | H:{h:.2f} | L:{l:.2f} | C:{c:.2f} | TS:{market_ts}")
                else:
                    logger.info(f"[MISSING] {ticker} - No data at TS:{market_ts}")

            except Exception as e:
                logger.info(f"[ERROR] {ticker} loop crash: {e}. Cooling down...")
...
async def main_engine():
    async with AsyncSession() as session:
        ...
        # Deploy all 10 workers in parallel
        tasks = [engine.process_asset(session, asset) for asset in ASSETS]
        await asyncio.gather(*tasks)

app = Flask(__name__)
@app.route("/")
def status(): return jsonify({"status": "active", "assets_count": len(ASSETS)}), 200
