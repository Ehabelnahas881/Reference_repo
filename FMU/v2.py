        ...
        with psycopg2.connect(host=hostname, dbname=database, user=username, password=password, port=port_id) as conn:
            ...
            with conn.cursor() as cur:
                ...
                if not financial_market_pks:
                    cur.execute('SELECT "financial_market_PK" FROM "dyLEARN".financial_market_list')
                    financial_market_pks = [row[0] for row in cur.fetchall()]
                    if not financial_market_pks:
                        return {'success': True, 'errors': ["No markets found in the database."]}
                ...
                if non_us_pks:
                    ...
                    if dataframe is not None:
                        ...
                        def correct_date_time(cleaned_df):
                            ...
                            def normalize_primary_exchanges(value):
                                if isinstance(value, list):
                                    return [item[1:-1] for sublist in value for item in sublist]
                                if isinstance(value, str):
                                    return [item.strip() for item in value.split(',') if item.strip()]
                                return value

                            cleaned_df['primary_exchanges'] = cleaned_df['primary_exchanges'].apply(normalize_primary_exchanges)
                            ...
                            cleaned_df['primary_exchanges'] = cleaned_df['primary_exchanges'].apply(lambda x: str(x).strip() if pd.notna(x) else x)
                            ...
                            object_cols = [
                                'financial_market_PK',
                                'financial_market_code',
                                'financial_market_name',
                                'financial_market_ID',
                                'financial_market_timezone_region_city_PK',
                            ]
                            for col in object_cols:
                                cleaned_df[col] = pd.Series([None] * len(cleaned_df), dtype=object)
                            missing_market_list = []
                            for i in range(len(cleaned_df)):
                                ...
                                for col in time_cols:
                                    ...
                                    dt_open_utc = dt_open_utc.tz_convert('UTC').tz_localize(None)
                        ...
                        for i in range(len(time_adjusted_dataframe)):
                            if time_adjusted_dataframe.loc[i, 'notes'] == "":
                                time_adjusted_dataframe.loc[i, 'notes'] = "-"
                        ...
                        time_adjusted_dataframe['current_status'] = time_adjusted_dataframe['current_status'].astype(object)
                        time_adjusted_dataframe['notes'] = time_adjusted_dataframe['notes'].astype(object)
                        ...
                        for i in range(len(time_adjusted_dataframe)):
                            ...
                            for target_date in dates_to_insert.iloc[:, 0]:
                                ...
                                if time_adjusted_dataframe.loc[i, 'current_status'] == "closed":
                                    time_adjusted_dataframe.loc[i, 'current_status'] = 0
                                else:
                                    time_adjusted_dataframe.loc[i, 'current_status'] = 1
                                if time_adjusted_dataframe.loc[i, 'notes'] is None:
                                    time_adjusted_dataframe.loc[i, 'notes'] = " "
                                ################################################
                        ...
                        if missing_pks:
                            ...
                            for market in markets:
                                ...
                                # استخراج القيم من الصف المجلوب من قاعدة البيانات
                                country_timezone_region_city_name = row['country_timezone_region_city_name']
                                country_timezone_region_city_PK = row['country_timezone_region_city_PK']
                                timezone_region_city_PK = row['timezone_region_city_PK']
                ...
                if us_pks:
                    ...
                    if dataframe is not None:
                        ...
                        date_cols = cleaned_df.columns[cleaned_df.columns.str.contains(pat = 'Date')].tolist()

                        for col in time_cols:
                            # Remove timezone abbreviations like ET, EST, etc.
                            cleaned_df[col] = cleaned_df[col].str.replace(r'\s+(ET|EST|EDT|CST|CDT|PST|PDT|GMT|UTC)$', '', regex=True)
                            cleaned_df[col] = pd.to_datetime(cleaned_df[col], errors='coerce')
                        
                        for col in date_cols:
                            # Split the date string if it contains multiple dates
                            cleaned_df[col] = cleaned_df[col].str.split().str[:3].str.join(' ') + ' 00:00 AM'
                        ...
                        if apicalls_time_adjusted_dataframe.empty:
                            print("No US markets found from API, skipping US processing")
                        else:
                        ...
                        for i in range(len(apicalls_time_adjusted_dataframe)):
                            if apicalls_time_adjusted_dataframe.loc[i, 'mrktStatus'] == "Open":
                                apicalls_time_adjusted_dataframe.loc[i, 'mrktStatus'] = 1
                            else:
                                apicalls_time_adjusted_dataframe.loc[i, 'mrktStatus'] = 0
                            ...
                            last_logged_date = result[0] if result and result[0] else date.today() - timedelta(days=1)
                            last_logged_date = last_logged_date + timedelta(days=1)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Run FMU/v2 processing for specified financial market PKs. If no PKs are provided, all markets are processed.'
    )
    parser.add_argument(
        'market_pks',
        nargs='*',
        type=int,
        help='Financial market PKs to process. Leave empty to process all markets.'
    )
    args = parser.parse_args()

    result = resolve_FMU(None, None, args.market_pks)
    print(result)