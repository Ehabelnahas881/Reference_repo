    ...
    def _writer_loop(self):
        ...
        while True:
            ...
            try:
                ...
                conn.commit()
                # Log to both main terminal and the specific database_ops.log
                msg = f"DATA INSERTED for {ticker} | at {full_timestamp}"
                logger.info(msg) 
                self.db_log.info(msg)
            except Exception as e:
                self.db_log.error(f"DB Write Error for {ticker}: {e}")
                conn.rollback()
                try: 
                    conn.close()
                    conn = self._get_conn()
...
def validate_market_session():
    ...
    start = datetime.strptime("16:08", "%H:%M").time()
    end = datetime.strptime("16:10", "%H:%M").time()
...
class SyncScraper:
    ...
    def fetch_price(self, session):
        try:
            ...
            el = soup.find(attrs={"data-testid": "qsp-price"}) #or soup.find(attrs={"data-field": "qsp-overnight-price"}) or soup.find(attrs={"data-field": "qsp-pre-price"})
