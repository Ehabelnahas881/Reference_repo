        ...
        while True:
            ...
            async with AsyncSession(impersonate=impersonate_key) as session:
                try:
                    ...
                    logger.info(f"[{ticker}] WORKER LIVE.")
                    ...
                    while (time.time() - session_born) < session_lifespan:
                        ...
                        for i in range(4):
                            ...
                            if i < 3: 
                                sleep_dur = schedule[i] - (time.time() - minute_loop_start)
                                if sleep_dur > 0: await asyncio.sleep(sleep_dur)

                        valid_prices = [v for v in prices if v is not None]
                        if len(valid_prices) == 4:
                            # MODIFIED: Use standard .put() for threading-queue
                            asset_queues[ticker].put({
                                "ticker": ticker,
                                "data": (ts_val.date(), ts_val, valid_prices[0], max(valid_prices[1:3]), min(valid_prices[1:3]), valid_prices[3]),
                                "attempts": 0
                            })
