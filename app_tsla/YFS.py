...
def get_market_session_config():
    """Returns the primary and secondary IDs to look for based on Time."""
    now_utc = datetime.now(UTC)
    curr = now_utc.hour + (now_utc.minute / 60)
    
    # Pre-Market (8:00 UTC to 13:30 UTC / 4:00 AM to 9:30 AM EST)
    if 8.0 <= curr < 13.5:
        return ["qsp-pre-price", "qsp-post-price", "qsp-price"], "Pre-Market"
    # Regular Market (13:30 UTC to 20:00 UTC / 9:30 AM to 4:00 PM EST)
    elif 13.5 <= curr < 20.0:
        return ["qsp-price"], "Regular"
    # Post-Market (20:00 UTC to 00:00 UTC / 4:00 PM to 8:00 PM EST)
    elif 20.0 <= curr <= 24.0:
...
class DataWorker:
    def __init__(self, ticker):
        ...
        self.session.headers.update({'Referer': 'https://finance.yahoo.com/', 'Connection': 'keep-alive'})
    ...
    def fetch_price(self):
        ...
        try:
            headers = {
                ...
                'If-None-Match': str(uuid.uuid4())
            }
            url = f"https://finance.yahoo.com/quote/{self.ticker}/?p={self.ticker}&_ts={time.time()}"
            ...
            # RECURSIVE SEARCH: Try every possible ID for the current session
            ...
            # ULTIMATE FALLBACK: Search for Yahoo's direct streamer component
            streamer = soup.find("fin-streamer", {"data-symbol": self.ticker})
            if streamer and streamer.get("value"):
                return float(streamer["value"].replace(',', ''))
    ...
    def run(self):
        self.logger.info(f"--- Worker Online (V3-Universal) for {self.ticker} ---")
        ...
        while True:
            try:
                ...
                if m_open is None:
                    self.logger.warning(f"Fetch Failed @ {curr_ts.strftime('%H:%M:%S')}")
                    continue
                
                m_high, m_low, m_close = m_open, m_open, m_open
                
                # Sample 5 times within the minute
                for _ in range(5):
    ...
    def _start():
        for aid in financial_asset_list:
            ...
            if ticker:
                ...
                time.sleep(45) # Stagger to prevent burst block
...
pkill -9 python3
nohup python3 omni_engine.py > engine_output.log 2>&1 &
...
# --- SELENIUM GUARD (Bypasses GDPR/Consent Walls) ---
class DriverPool:
    def __init__(self):
        ...
        # Ensure these paths match your environment (e.g., /usr/bin/chromium)
        opts.binary_location = "/usr/bin/chromium"
        
        try:
            self.driver = webdriver.Chrome(service=Service(executable_path="/usr/bin/chromedriver"), options=opts)
    ...
    def get_session_data(self, ticker):
        with self.lock:
            ...
            # guccounter=1 is a bypass flag for Yahoo's consent redirect
            self.driver.get(f"https://finance.yahoo.com/quote/{ticker}?guccounter=1")
            time.sleep(8) 
            
            # Specifically handle the German "Alle akzeptieren" button if it appears
            try:
...
# --- MARKET SESSION LOGIC (Saved Logic) ---
def get_market_session_config():
    now_utc = datetime.now(UTC)
    curr = now_utc.hour + (now_utc.minute / 60)
...
# --- DATA WORKER (Parallel & Efficient) ---
class DataWorker:
    def __init__(self, ticker):
        self.ticker = ticker
        self.session = requests.Session()
    ...
    def sync_session(self):
        """Injects Selenium cookies into the Requests session."""
        try:
    ...
    def fetch_price(self):
        target_ids, _ = get_market_session_config()
        try:
            # Use guccounter to help prevent redirects back to privacy pages
            ...
            # Logic: Try IDs first, then fallback to JSON re-scrape
            soup = BeautifulSoup(resp.text, 'lxml')
            for tid in target_ids:
                el = soup.find(attrs={"data-testid": tid})
                if el and el.text:
            ...
            # Regex Fallback for hidden price in scripts
    ...
    def run(self):
        print(f"[SYSTEM] Worker for {self.ticker} Online.", flush=True)
        self.sync_session()
        next_min = (datetime.now(UTC) + timedelta(minutes=1)).replace(second=0, microsecond=0)
        ...
        while True:
            try:
                ...
                if m_open is None:
                    print(f"[WARN] {self.ticker} Fetch Failed. Resyncing session...", flush=True)
                    self.sync_session()
                    continue
                
                m_high, m_low, m_close = m_open, m_open, m_open
                
                # Sampling logic (5 ticks per minute)
                for _ in range(5):
                    time.sleep(random.uniform(8.0, 10.0))
                    tick = self.fetch_price()
                    if tick:
                ...
                # Periodic resync every 20 minutes to keep session alive
    ...
    def _start():
        ...
        for aid in financial_asset_list:
            ...
            if ticker:
                ...
                time.sleep(10) # Stagger boot to prevent race conditions
    threading.Thread(target=_start, daemon=True).start()
    return {"success": True}
