                ...
                if non_us_pks:
                    ...
                    if dataframe is not None:
                        ...
                        def correct_date_time(cleaned_df):
                            ...
                            # --- الحل الصحيح ---
                            # 1. تأكد أن العمود من نوع 'object' ليقبل القوائم
                            cleaned_df['primary_exchanges'] = cleaned_df['primary_exchanges'].astype(object)

                            # 2. قم بعمل split للعمود بالكامل دفعة واحدة (سيحول كل خلية إلى قائمة)
                            cleaned_df['primary_exchanges'] = cleaned_df['primary_exchanges'].str.split(',')

                            # 3. الآن استخدم explode لتوزيع القوائم على صفوف جديدة
                            cleaned_df = cleaned_df.explode('primary_exchanges', ignore_index=True)

                            # 4. تنظيف المسافات الزائدة الناتجة عن الـ split
...
#print("Starting FMU Financial Market Session Time Logger...")   
#result = resolve_FMU(None, None, None)
# print(result)
