...
from settings import USER, PASSWORD, DATABASE, HOST, PORT, USER_AGENT, HEADERS
# Imports
...
# --- LOGGING AUTOMATION ---
# This finds the absolute path of the directory where YFS.py is located
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
LOG_DIR = os.path.join(BASE_DIR, "logs")
MAIN_LOG = os.path.join(BASE_DIR, "market_sync.log")

# Automatically create the folder if it doesn't exist
if not os.path.exists(LOG_DIR):
    try:
        os.makedirs(LOG_DIR)
        print(f"Successfully created: {LOG_DIR}")
...
def get_asset_logger(ticker):
    """Creates a dedicated logger for a specific asset ticker."""
    asset_logger = logging.getLogger(f"Log-{ticker}")
    
    if not asset_logger.handlers:
        asset_logger.setLevel(logging.INFO)
        
        # Use the absolute path defined above
        file_path = os.path.join(LOG_DIR, f"{ticker}.log")
        ...
        asset_logger.addHandler(file_h)
        asset_logger.addHandler(console_h)
        asset_logger.propagate = False
...
class AsyncDBWriter:
    ...
    def __init__(self, config):
        ...
        # Specific DB Logger
        db_log_path = os.path.join(LOG_DIR, "database_ops.log")
        self.db_log = logging.getLogger("DB-Writer")

        if not self.db_log.handlers:
            h = logging.FileHandler(db_log_path)
            h.setFormatter(logging.Formatter(log_format, date_format))
            self.db_log.addHandler(h)
            self.db_log.addHandler(logging.StreamHandler())
    ...
    def _writer_loop(self):
        ...
        while True:
            ...
            # Ensure connection exists before handling tasks
            if conn is None or conn.closed != 0:
                try:
                    self.db_log.info("Opening a new PostgreSQL connection pipe...")
                    conn = self._get_conn()
                except Exception as conn_err:
                    self.db_log.error(f"Failed to connect to database: {conn_err}. Retrying in 5s...")
                    self.queue.task_done()
                    # Put it back at the top of the queue or drop? Re-queue to avoid data loss.
                    self.queue.put((ticker, row))
                    time.sleep(5)
                    continue
            ...
            try:
                ...
                msg = f"DATA INSERTED for {ticker} | at {full_timestamp}"
                logger.info(msg) 
                self.db_log.info(msg)
                
            except (psycopg2.OperationalError, psycopg2.InterfaceError) as net_err:
                # Catch closed socket immediately without forcing a broken rollback
                self.db_log.error(f"Network pipe broken for {ticker}: {net_err}. Resetting connection.")
                conn = None # Nullify connection to trigger reconnect on next loop iteration
                self.queue.put((ticker, row)) # Save the data point back to queue
                time.sleep(2)
                
            except Exception as e:
                self.db_log.error(f"SQL execution error for {ticker}: {e}")
                # Safe rollback check: Only rollback if the pipeline itself isn't dead
...
def validate_asset_market_day(ticker):
    """Checks if today is a trading day for a SPECIFIC asset's market."""
    try:
        conn = psycopg2.connect(**db_config)
        with conn.cursor() as cur:
            # Your new query to link assets to their specific market schedules
            query = """
                SELECT 
                    s.next_market_trading_session_date
            ...
            if row and row[0]:
                ...
                # If the database returns a datetime, convert it to a date object
                if isinstance(next_session, datetime):
                    next_session = next_session.date()
                # Returns True if today is a valid session for this specific market
                return next_session >= datetime.now(UTC).date()
        return False
    except Exception as e:
        logger.error(f"[{ticker}] Market Check Error: {e}")
        return False
    finally:
...
class SyncScraper:
    ...
    def fetch_price(self, session):
        try:
            ...
            if resp.status_code != 200: return "unexpected_IP_status" , resp.status_code
            soup = BeautifulSoup(resp.text, 'lxml')
            el = soup.find(attrs={"data-testid": "qsp-post-price"}) or soup.find(attrs={"data-field": "qsp-overnight-price"}) or soup.find(attrs={"data-field": "qsp-price"})
            return float(re.sub(r'[^\d.]', '', el.text)) if el else None
        except Exception as e:
            logger.error(f"[{self.ticker}] Exception during fetch_price: {e}")
            return None

def run(ticker,worker_log):
    ...
    with requests.Session(impersonate=fp["impersonate"]) as session:
        ...
        while datetime.now() < session_expiry:
            ...
            if len(prices) == 4:
                ...
                worker_log.info(f"[{ticker}] Data collected and pushed to DB: O:{row[2]} H:{row[3]} L:{row[4]} C:{row[5]}")
                ...
                # Add this line to immediately know if 0 data points were successfully parsed
                logger.warning(f"[{ticker}] No price points collected during this minute interval.")
...
def asset_worker_thread(ticker):
    # Initialize the specific logger for this ticker
    # This also handles the 'asset_name is not defined' error by using 'ticker'
    worker_log = get_asset_logger(ticker)
    
    worker_log.info(f"[{ticker}] Worker validated.")
    standby_msg_logged = False
    
    while True:
        is_market_day = validate_asset_market_day(ticker)
        is_market_session = validate_market_session()

        if is_market_day and is_market_session:
            worker_log.info(f"[{ticker}] Market is OPEN. Starting scraper...")
            try:
                run(ticker,worker_log) 
            except Exception as e:
                worker_log.error(f"[{ticker}] Scraper error: {e}")
                time.sleep(60)
        else:
            now_utc = datetime.now(UTC).strftime('%H:%M')
            time.sleep(120)
            worker_log.info(f"[{ticker}] Standby ({now_utc} UTC). Market closed or outside hours. Rechecking in 5m...")
